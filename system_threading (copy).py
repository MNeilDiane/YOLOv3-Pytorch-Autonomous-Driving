...
class Main():
    ...
    def __init__(self):
        ...
        self.Button0 = tk.Button(self.win, text='System Activate', image=im_button0, compound=tk.CENTER,
                            font=('Time New Roman', 12), fg='#99ff66',
                            width=10, height=3, relief=RAISED, command=lambda:self.System_Activate())
        ...
        self.Button5 = tk.Button(self.win, text='Exit', image=im_button5, compound=tk.CENTER,
                                 font=('Time New Roman', 12), fg='red', width=10, height=3, relief=RAISED,
        ...
        self.label1.place(x=950, y=43, width=330, height=40)

        #Entrys
        self.Entry3 = tk.Entry(self.win, textvariable=self.integer, justify='center', font=('Time New Roman', 12),
                              bg='#c0c0c0', fg='#99ff66')
        self.Entry3.place(x=550, y=50, width=200, height=50)
        # Text
        self.text1 = tk.Text(self.win, bg='#c0c0c0')
        self.text1.place(x=950, y=83, width=330, height=600)
        self.win.mainloop()
        ...
        if self.order == b'1':
            ...
            while (self.order==b'1'):
                ...
                if self.winisvisible == 1:
                    # Canvos
                    self.canvas1 = tk.Canvas(self.win, bg='white', width=600, height=480)
        ...
        if self.order == 1:
            ...
            while self.capture1.IsGrabbing():
                ...
                if grabResult.GrabSucceeded():
                    ...
                    self.integerl.set(label)
                    self.update()
                    self.r.hset("LEFT", "left", str(label))
                    print(label)
                    # 进行检测
                    if self.winisvisible == 1:
                        # Canvos
        ...
        if self.order == -1:
            ...
            while self.capture1.IsGrabbing():
                ...
                if grabResult.GrabSucceeded():
                    ...
                    self.integerl.set(label)
                    self.update()
                    self.r.hset("RIGHT", "right ", str(label))
                    print(label)
                    # 进行检测
                    if self.winisvisible == 1:
                        # Canvos
    ...
    '''
    Thread2 wait the key to activate the  rear left detection
    '''
